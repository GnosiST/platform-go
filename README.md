...
能力装停卸策略由 [resources/platform-capability-operation-policy.json](resources/platform-capability-operation-policy.json) 约束：当前支持启动前按 profile/`PLATFORM_CAPABILITIES`/`PLATFORM_CAPABILITY_LOCK_FILE`/下游 composition root 启用，不支持运行时热插拔或通用破坏性卸载；禁用后合同表面不应再暴露，基础能力不可卸载。

插件管理 v1 的机器合同在 [resources/platform-plugin-management-v1.json](resources/platform-plugin-management-v1.json)：它采用 restart-required desired-state model。安装或启用插件等于通过 profile、`PLATFORM_CAPABILITIES`、`PLATFORM_CAPABILITY_LOCK_FILE` 或下游 composition root 声明期望能力集合，重新生成合同并手动重启服务；禁用等于从期望集合移除并重启生效。v1 不从远端仓库拉代码，不支持 runtime hot install/uninstall，不提供破坏性卸载按钮，也不集成 WebSocket；状态提示使用 HTTP polling、`version.json` 或 API version check。
