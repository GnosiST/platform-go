...
Production transport is initialized with `PLATFORM_PUBLIC_BASE_URL`, `PLATFORM_TRUSTED_PROXIES` and `PLATFORM_HTTP_MAX_BODY_BYTES`. The public base must be an HTTPS origin, trusted proxies must be exact IPs or narrow CIDRs, and the standard Admin container binds `127.0.0.1:8080` behind a reviewed external TLS edge. The API ignores forwarded HTTPS headers from all other peers; do not expose the origin port directly or configure all-address proxy ranges.
