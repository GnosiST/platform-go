...
The persistent full-scope unfinished inventory, in task-graph order, is `multi-datasource-contract-and-runtime`, `tenant-placement-and-request-routing`, `datasource-read-write-routing`, `sharding-and-tenant-migration`, `federated-read-query`, `xa-optional-adapter`, `database-certification-matrix`, `transactional-outbox-and-one-mq-adapter`, `asynchronous-search-projection`, `open-source-portability`, `public-docs-community`, `public-docs-site` and `github-release-publication`. This inventory is not the v0.1.0 execution order: the nine data-plane and integration nodes are deferred until after release. The menu tree/detail workbench, directory/page authoring, typed page parameters, page-button metadata, full organization E2E and unified error-code governance are implemented with their focused contract, browser and governance evidence. Target menu serving and role-menu migration writes remain disabled by the existing migration policy.

For v0.1.0, release blockers are `open-source-portability`, `public-docs-community`, `public-docs-site` and `github-release-publication`. The nine datasource/routing/sharding/federation/XA/certification/outbox/search nodes remain visible as post-release optional `deferred` work and still block the persistent full-scope objective. The v0.1.0 support boundary is one datasource and one native transaction boundary: no tenant/request datasource routing, automatic read/write routing, sharding or tenant migration, federated query, XA, shipped MQ adapter or search projection. MySQL and PostgreSQL are wired targets but not blanket-certified; SQLite is development/test-only by support policy, and Oracle and KingbaseES are unsupported. `alibaba/page-agent` is only a default-off optional `public-docs-site` sub-capability and is never included in Admin or API by default.

`platform-service-contract-standard` adds a validated `capability.Manifest.Service` surface, trusted tenant-context fencing, five-plane declarations, versioned event envelopes, W3C trace schemas, deterministic OpenAPI/AsyncAPI and isolated Go/TypeScript SDK artifacts. `unified-error-code-governance` closes the registry-backed HTTP error, correlation, redaction and generated-contract gate; event delivery and OpenTelemetry export remain separate. `persisted-query-command-object-runtime` now adds a server-owned versioned registry, typed arguments, Casbin and trusted scope enforcement, logical Query/Command ASTs, cost and timeout budgets, parameterized GORM execution, persistent idempotency, conditional Admin transport and generated OpenAPI/TypeScript contracts. Workload identity protocols, event delivery, datasource routing, federation and XA remain separate unfinished work.

`integration-ports-disabled-default` adds business-neutral message bus and search ports, explicit disabled errors, redacted health states and production startup gates. The stock process still ships no vendor adapter, Outbox, DLQ, replay worker or search projection.
...
```bash
proxy sh -lc 'PLATFORM_HTTP_ADDR=127.0.0.1:19200 go run ./cmd/platform-api & pid=$!; trap "kill $pid 2>/dev/null || true" EXIT; for i in $(seq 1 20); do curl -fsS http://127.0.0.1:19200/api/health && exit 0; sleep 0.5; done; exit 1'
```
...
```bash
node scripts/generate-admin-resource-contract.mjs
go run ./cmd/platform-contracts admin-resources --output resources/generated/admin-capability-resource-contract.json
go run ./cmd/platform-contracts app-routes --output resources/generated/app-route-contract.json
go run ./cmd/platform-contracts audit --output resources/generated/platform-capability-audit.json
node scripts/generate-admin-openapi.mjs
node scripts/generate-admin-codegen-preview.mjs
node scripts/generate-admin-scaffold-plan.mjs
node scripts/generate-admin-scaffold-files.mjs
node scripts/generate-admin-scaffold-draft.mjs
node scripts/generate-admin-scaffold-promotion-review.mjs
node scripts/generate-app-openapi.mjs
node scripts/generate-app-codegen-preview.mjs
node scripts/generate-platform-operations-plan.mjs
node scripts/generate-production-auth-promotion-review.mjs
node scripts/generate-platform-promotion-evidence-templates.mjs
node scripts/generate-platform-promotion-evidence-package-draft.mjs
node scripts/validate-admin-resources.mjs
node scripts/validate-platform-governance-topology.mjs
node scripts/validate-platform-form-schema-layout-slots.mjs
node scripts/validate-platform-capability-contracts.mjs
node scripts/validate-platform-capability-profiles.mjs
node scripts/validate-platform-reference-discovery.mjs
node scripts/validate-platform-reference-coverage.mjs
node scripts/validate-platform-admin-api-boundary.mjs
node scripts/validate-platform-app-client-api-boundary.mjs
node scripts/validate-platform-personnel-runtime-readiness.mjs
node scripts/validate-platform-engineering-capabilities.mjs
node scripts/validate-platform-codegen-source-writing-readiness.mjs
node scripts/validate-platform-cache-invalidation.mjs
node scripts/validate-platform-deployment-topology.mjs
node scripts/validate-platform-foundation-task-graph.mjs
node scripts/validate-platform-task-execution-audit.mjs
node scripts/validate-platform-goal-completion-audit.mjs
node scripts/validate-platform-node-closeout-audit.mjs
node scripts/validate-platform-objective-conformance.mjs
node scripts/validate-platform-sensitive-data-migration.mjs
node scripts/validate-platform-promotion-evidence-templates.mjs
node scripts/validate-platform-promotion-evidence-package.mjs --package <promotion-evidence-package>
node scripts/validate-platform-file-storage-experience.mjs
node scripts/validate-platform-refresh-token-family-promotion.mjs
...
node scripts/validate-admin-i18n.mjs
node scripts/validate-admin-ui-contracts.mjs
node --test scripts/admin-ui-contracts.test.mjs
...
Run `node scripts/generate-platform-operations-plan.mjs`, `node scripts/generate-production-auth-promotion-review.mjs` and `node scripts/validate-platform-production-readiness.mjs` before release or production configuration work. Use `node scripts/run-platform-production-preflight.mjs --list` to inspect the declared preflight catalog, `node scripts/run-platform-production-preflight.mjs --policy <policy-id>` to dry-run a policy-specific command set, and add `--run` only when the operator is ready to execute the selected checks. Production operation policies and their policy-level preflight requirements are contract-first and non-mutating by default:
