...
The original 37-node foundation baseline remains implemented and closed. The active completion program now contains 29 nodes beyond that baseline; fourteen continuation nodes, including `platform-service-contract-standard`, `persisted-query-command-object-runtime`, `integration-ports-disabled-default`, the organization/RBAC/menu contract gate, the organization role-pool backend/migration runtime, `organization-user-admin-experience` and `role-tree-and-authorization-entry`, are `implemented`, so the current governance state is `66 total / 51 implemented / 15 controlled unfinished` with `completionStatus=not-complete-controlled`.
...
The exact unfinished order is `menu-tree-and-button-permission-configuration`, `organization-rbac-menu-e2e-qa`, `multi-datasource-contract-and-runtime`, `tenant-placement-and-request-routing`, `datasource-read-write-routing`, `sharding-and-tenant-migration`, `federated-read-query`, `xa-optional-adapter`, `database-certification-matrix`, `transactional-outbox-and-one-mq-adapter`, `asynchronous-search-projection`, `open-source-portability`, `public-docs-community`, `public-docs-site` and `github-release-publication`. The role workbench now provides a strict role-group-to-role tree, reviewed move/disable flows and atomic allow/deny/data-scope authorization. Independent page-only `role_menu` persistence, revision-aware target resolution and migration comparison seams are implemented, but the menu assignment entry remains read-only while the write gate is closed and the menu governance UI and full cutover evidence remain unfinished. The [activated topology](docs/superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md) remains the authority for boundaries and dependencies; the organization contract and implemented Admin boundary are [documented separately](docs/platform-organization-rbac-menu-contract.md).
