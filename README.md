...
The original 37-node foundation baseline remains implemented and closed. The active completion program now contains 30 nodes beyond that baseline; fifteen continuation nodes, including `platform-service-contract-standard`, `persisted-query-command-object-runtime`, `integration-ports-disabled-default`, the organization/RBAC/menu contract gate, the organization role-pool backend/migration runtime, `organization-user-admin-experience`, `role-tree-and-authorization-entry` and `menu-tree-and-button-permission-configuration`, are `implemented`, so the current governance state is `67 total / 52 implemented / 15 controlled unfinished` with `completionStatus=not-complete-controlled`.
...
The persistent full-scope unfinished inventory, in task-graph order, is `organization-rbac-menu-e2e-qa`, `unified-error-code-governance`, `multi-datasource-contract-and-runtime`, `tenant-placement-and-request-routing`, `datasource-read-write-routing`, `sharding-and-tenant-migration`, `federated-read-query`, `xa-optional-adapter`, `database-certification-matrix`, `transactional-outbox-and-one-mq-adapter`, `asynchronous-search-projection`, `open-source-portability`, `public-docs-community`, `public-docs-site` and `github-release-publication`. This inventory is not the v0.1.0 execution order: the nine data-plane and integration nodes are deferred until after release. The menu tree/detail workbench, directory/page authoring, typed page parameters and page-button metadata are implemented with six-viewport evidence. Target menu serving and role-menu migration writes remain disabled; full Tree Transfer scale, principal-level dual-read equivalence, migration cutover/rollback and organization E2E remain owned by `organization-rbac-menu-e2e-qa`.

For v0.1.0, release blockers are `organization-rbac-menu-e2e-qa`, `open-source-portability`, `public-docs-community`, `public-docs-site` and `github-release-publication`. The nine datasource/routing/sharding/federation/XA/certification/outbox/search nodes remain visible as post-release optional `deferred` work and still block the persistent full-scope objective. The v0.1.0 support boundary is one datasource and one native transaction boundary: no tenant/request datasource routing, automatic read/write routing, sharding or tenant migration, federated query, XA, shipped MQ adapter or search projection. MySQL and PostgreSQL are wired targets but not blanket-certified; SQLite is development/test-only by support policy, and Oracle and KingbaseES are unsupported. `alibaba/page-agent` is only a default-off optional `public-docs-site` sub-capability and is never included in Admin or API by default.
