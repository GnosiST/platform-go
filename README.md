...
能力装停卸策略由 [resources/platform-capability-operation-policy.json](resources/platform-capability-operation-policy.json) 约束：当前支持启动前按 profile/`PLATFORM_CAPABILITIES`/下游 composition root 启用，不支持运行时热插拔或通用破坏性卸载；禁用后合同表面不应再暴露，基础能力不可卸载。
