...
| Organization And Area Model | tenants, org units, role groups, area codes and user/role linkage fields | implemented base | `TestCurrentSessionEndpointReturnsRoleBackedPermissions` covers current-session tenant/org/area context; store tests cover org/area data scopes; `organization-user-admin-experience`, the focused role workbench, menu workbench and organization E2E gates are accepted, while production menu serving and role-menu writes remain default-closed pending explicit rollout approval |
...
- `org-units` remains the shared institution, department and team tree. Every organization belongs to one tenant. The activated target gives each ordinary user one primary organization and derives the user's tenant on the server; direct editable `users.tenantCode` is migration-source behavior, not the retained form contract. Platform principals without an organization are an explicit exception class.
- `role-groups` remains classification and isolation metadata only. The activated target is a strict two-level `role group -> role` tree: role groups are never nested, each role has exactly one required group, organizations bind multiple enabled same-tenant role groups, and user roles must be a subset of the organization's effective role pool. Role groups never grant permissions, own membership, inherit policies or carry data scopes.
- `area-codes` is the shared global address-code and region master-data resource. Tenants, org units, users and optional personnel resources should carry `areaCode` when regional ownership matters. It keeps one logical tree/table with `parentCode`, `path`, data-driven `level`, numeric `depth` and source provenance fields, so deployments can initialize global, China-specific or exhibition-market datasets without constraining codes to one country's administrative format or changing platform code. Detailed address fields, employment locations or regional operation workflows should stay in the owning capability until at least two reusable platform capabilities need the same contract.
- Authorization still comes from roles, explicit allow/deny permissions and data scopes, with Casbin as the final API boundary. Menu assignment moves to independent `role_menu` data: menus decide navigable visibility, while API and page-button permissions remain separate permission resources. Existing `menu.permission` behavior is migration source only and must pass backfill, dual-read comparison and cutover before deprecation.

The 2026-07-14 topology activation supersedes only the old organization/RBAC/menu target semantics; it does not add a second organization model. `org-units`, tenants, users, roles, role groups and area codes remain non-droppable primitives. The six organization nodes cover `org_unit_role_groups`, single-group role ownership, tenant derivation, `role_menu`, directory/page menus, separate page-button/API permissions and their Admin experiences. Contract, backend migration, organization/user UI, strict role tree, menu configuration and browser E2E are closed. Production serving cutover and role-menu migration writes remain default-closed until an explicit rollout approval. Personnel files, positions and assignments stay optional capability resources and reuse the shared tenant and organization boundary.

The governance-topology gate now records direct user-tenant editing, nested role-group metadata and permission-derived menus as migration-source behavior. Its activated target requires tenant plus organization ownership, strict non-nested role groups, organization role pools, server-derived user tenant, independent role-menu assignment and role-owned permission/data-scope policy. Address-code references remain ownership metadata and never grant access by themselves.
...
| Tenants, users, org units, user-role bindings and role groups | Platform foundation | Current normalized resources plus activated migration nodes for organization role-group bindings, role pools and tenant derivation | Each role belongs to one non-nested group; organizations bind same-tenant groups; users receive only roles inside their organization pool. |
| Permissions, menus and API resources | Platform foundation | Current permission-derived menus plus activated `role_menu`, directory/page menu and page-button migration | Menu assignment controls navigation; API/page-button permissions remain separate resources and Casbin remains authoritative for APIs. |
...
The P1/P2 foundation items below are represented in `resources/platform-foundation-task-graph.json`. The original 37-node foundation baseline and completed continuation nodes remain closed, producing `67 total / 58 implemented / 9 controlled unfinished`; `resources/platform-foundation-alignment-audit.json` keeps the exact nine-node deferred remainder in `requiredFutureTaskNodes` and tracks the implemented v0.1.0 publication gate in `requiredTaskNodes` without reopening any implemented closeout.
...
`sensitive-data-protection-runtime` is `implemented`; field classification remains manifest-driven and protected persistence continues to reject plaintext fallback.

`sensitive-data-historical-migration` is `implemented`; it remains an offline maintenance workflow and does not add HTTP migration routes or plaintext dual-read behavior.

Sensitive-data historical migration closeout:
...
Sensitive-data reveal closeout:
...
3. Generator hardening
   ...
   - Current state: `resources/platform-task-execution-audit.json` records the exact ordered nine-node deferred remainder followed by the pending v0.1.0 publication gate, plus cross-lane dependencies, domain locks and approved parallel windows. Runtime/source-writing promotion blockers remain explicit; operational UI work stays tied to Product Design and browser evidence.
   - Current state: `resources/platform-goal-completion-audit.json` records `not-complete-controlled` at 67 total, 58 implemented and 9 unfinished. `resources/platform-node-closeout-audit.json` preserves 58 implemented closeouts and rejects closeout evidence for unfinished nodes. `resources/platform-objective-conformance.json` projects the same nine controlled blockers.
...
The original foundation baseline and completed continuation nodes remain closed. The active graph is `67 total / 58 implemented / 9 controlled unfinished`; external production promotion remains `not-approved`, v0.1.0 publication evidence is verified, and runtime mutation, refresh-token-family default runtime, source writing, uncertified databases, federation mutation, default XA and messaging/search adapters remain disabled.
