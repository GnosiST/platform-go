...
The P1/P2 foundation items below are represented in `resources/platform-foundation-task-graph.json`. The original 37-node foundation baseline remains implemented and closed, including Task 8 production Admin OIDC evidence. `runtime-security-containment`, `admin-watermark-export-governance`, `sensitive-data-protection-runtime`, `sensitive-data-historical-migration` and `mask-strategy-runtime` are `implemented`, producing `53 total / 42 implemented / 11 controlled unfinished`; `resources/platform-foundation-alignment-audit.json` keeps the eleven remaining nodes in `requiredFutureTaskNodes` while keeping the implemented sensitive-data and masking nodes in the required task set.

Remaining nodes, in task-graph order: `sensitive-data-reveal-step-up`, `data-lifecycle-retention`, `multi-datasource-contract-and-runtime`, `database-certification-matrix`, `integration-ports-disabled-default`, `transactional-outbox-and-one-mq-adapter`, `asynchronous-search-projection`, `open-source-portability`, `public-docs-community`, `public-docs-site`, `github-release-publication`.

The first eight are newly governed data-protection, lifecycle, database-portability and optional-integration nodes. The final four are the existing open-source portability, community, public-site and GitHub-publication nodes. All twelve remain pending and retain their external certification, design-evidence and publication boundaries.
...
3. Generator hardening
   ...
   - Current state: `resources/platform-task-execution-audit.json` records the ordered eleven-node remainder of the completion program as controlled unfinished work. Future runtime/source-writing promotion blockers, required validators and conflict-free parallel batches remain explicit; visual/UI work stays tied to both `superpowers:brainstorming` and `product-design` evidence.
   - Current state: `resources/platform-goal-completion-audit.json` records `not-complete-controlled` at 53 total, 42 implemented and 11 unfinished. `resources/platform-node-closeout-audit.json` records 42 implemented closeouts and rejects closeout evidence for pending nodes. `resources/platform-objective-conformance.json` projects the same eleven controlled blockers while keeping approved stack, reference-only `zshenmez` usage, capability-contract governance, future promotion gates, visual gate order, Vercel boundary and production preflight aligned. Routine small nodes use focused cleanup evidence; `neat-freak` is reserved for phase, major-task and release closeout.
