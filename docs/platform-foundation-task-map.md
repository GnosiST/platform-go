...
The P1/P2 foundation items below are represented in `resources/platform-foundation-task-graph.json`. The original 37-node foundation baseline and fourteen completed continuation nodes remain closed, producing `66 total / 51 implemented / 15 controlled unfinished`; `resources/platform-foundation-alignment-audit.json` keeps the exact 15-node remainder in `requiredFutureTaskNodes` without reopening any implemented closeout.

Remaining nodes, in task-graph order: `menu-tree-and-button-permission-configuration`, `organization-rbac-menu-e2e-qa`, `multi-datasource-contract-and-runtime`, `tenant-placement-and-request-routing`, `datasource-read-write-routing`, `sharding-and-tenant-migration`, `federated-read-query`, `xa-optional-adapter`, `database-certification-matrix`, `transactional-outbox-and-one-mq-adapter`, `asynchronous-search-projection`, `open-source-portability`, `public-docs-community`, `public-docs-site`, `github-release-publication`.

The 15 unfinished nodes remain governed by the [activated remaining-task topology](superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md). The service contract, persisted Query/Command runtime, disabled integration ports, [organization/RBAC/menu contract](platform-organization-rbac-menu-contract.md), organization role-pool backend/migration runtime, organization/user Admin experience and role tree/authorization entry are closed; `menu-tree-and-button-permission-configuration` is the next organization-lane node while datasource work retains its independent dependencies and locks.
...
The original foundation baseline and fourteen completed continuation nodes remain closed. The active graph is `66 total / 51 implemented / 15 controlled unfinished`; external production promotion remains `not-approved`, and runtime mutation, refresh-token-family default runtime, source writing, uncertified databases, federation mutation, default XA and messaging/search adapters remain disabled.
