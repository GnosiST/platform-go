...
The P1/P2 foundation items below are represented in `resources/platform-foundation-task-graph.json`. The original 37-node foundation baseline and fifteen completed continuation nodes remain closed, producing `67 total / 52 implemented / 15 controlled unfinished`; `resources/platform-foundation-alignment-audit.json` keeps the exact 15-node remainder in `requiredFutureTaskNodes` without reopening any implemented closeout.

Remaining nodes, in task-graph order: `organization-rbac-menu-e2e-qa`, `unified-error-code-governance`, `multi-datasource-contract-and-runtime`, `tenant-placement-and-request-routing`, `datasource-read-write-routing`, `sharding-and-tenant-migration`, `federated-read-query`, `xa-optional-adapter`, `database-certification-matrix`, `transactional-outbox-and-one-mq-adapter`, `asynchronous-search-projection`, `open-source-portability`, `public-docs-community`, `public-docs-site`, `github-release-publication`.

The 15 unfinished nodes remain governed by the [activated remaining-task topology](superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md). Menu tree and button configuration is closed; organization E2E/cutover and unified error-code governance are the first release blockers. Six release blockers and nine post-release optional deferred nodes form disjoint machine-checked lanes, while all 15 remain blockers for the persistent full-scope objective.
...
3. Generator hardening
   ...
   - Current state: `resources/platform-task-execution-audit.json` records the exact ordered 15-node remainder, cross-lane dependencies, domain locks and approved parallel windows. Runtime/source-writing promotion blockers remain explicit; operational UI work stays tied to Product Design and browser evidence.
   - Current state: `resources/platform-goal-completion-audit.json` records `not-complete-controlled` at 67 total, 52 implemented and 15 unfinished. `resources/platform-node-closeout-audit.json` preserves 52 implemented closeouts and rejects closeout evidence for unfinished nodes. `resources/platform-objective-conformance.json` projects the same 15 controlled blockers.
...
The original foundation baseline and fifteen completed continuation nodes remain closed. The active graph is `67 total / 52 implemented / 15 controlled unfinished`; external production promotion remains `not-approved`, and runtime mutation, refresh-token-family default runtime, source writing, uncertified databases, federation mutation, default XA and messaging/search adapters remain disabled.
