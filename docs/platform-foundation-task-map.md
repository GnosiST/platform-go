...
The P1/P2 foundation items below are represented in `resources/platform-foundation-task-graph.json`. The original 37-node foundation baseline remains implemented and closed, including Task 8 production Admin OIDC evidence. `runtime-security-containment`, `admin-watermark-export-governance`, `sensitive-data-protection-runtime`, `sensitive-data-historical-migration`, `mask-strategy-runtime` and `sensitive-data-reveal-step-up` are `implemented`, producing `53 total / 43 implemented / 10 controlled unfinished`; `resources/platform-foundation-alignment-audit.json` keeps the ten remaining nodes in `requiredFutureTaskNodes` while keeping the implemented sensitive-data, masking and reveal nodes in the required task set.

Remaining nodes, in task-graph order: `data-lifecycle-retention`, `multi-datasource-contract-and-runtime`, `database-certification-matrix`, `integration-ports-disabled-default`, `transactional-outbox-and-one-mq-adapter`, `asynchronous-search-projection`, `open-source-portability`, `public-docs-community`, `public-docs-site`, `github-release-publication`.

The first six are newly governed lifecycle, database-portability and optional-integration nodes. The final four are the existing open-source portability, community, public-site and GitHub-publication nodes. All ten remain pending and retain their external certification, design-evidence and publication boundaries.

The approved program is defined by the [completion program](superpowers/specs/2026-07-12-platform-go-completion-program-design.md), [runtime security](superpowers/specs/2026-07-12-runtime-security-hardening-design.md), [watermark/export governance](superpowers/specs/2026-07-12-admin-watermark-export-design.md), [sensitive data encryption](superpowers/specs/2026-07-12-sensitive-data-encryption-design.md), [mask strategy runtime](superpowers/specs/2026-07-13-mask-strategy-runtime-design.md), [sensitive data reveal step-up](superpowers/specs/2026-07-13-sensitive-data-reveal-step-up-design.md) and [open-source docs/site](superpowers/specs/2026-07-12-open-source-docs-site-design.md) specifications.
...
3. Generator hardening
   ...
   - Current state: `resources/platform-task-execution-audit.json` records the ordered ten-node remainder of the completion program as controlled unfinished work. Future runtime/source-writing promotion blockers, required validators and conflict-free parallel batches remain explicit; visual/UI work stays tied to both `superpowers:brainstorming` and `product-design` evidence.
   - Current state: `resources/platform-goal-completion-audit.json` records `not-complete-controlled` at 53 total, 43 implemented and 10 unfinished. `resources/platform-node-closeout-audit.json` records 43 implemented closeouts and rejects closeout evidence for pending nodes. `resources/platform-objective-conformance.json` projects the same ten controlled blockers while keeping approved stack, reference-only `zshenmez` usage, capability-contract governance, future promotion gates, visual gate order, Vercel boundary and production preflight aligned. Routine small nodes use focused cleanup evidence; `neat-freak` is reserved for phase, major-task and release closeout.
