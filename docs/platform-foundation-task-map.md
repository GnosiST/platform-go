...
The P1/P2 foundation items below are represented in `resources/platform-foundation-task-graph.json`. The original 37-node foundation baseline and ten completed continuation nodes remain closed, producing `66 total / 47 implemented / 19 controlled unfinished`; `resources/platform-foundation-alignment-audit.json` keeps the exact 19-node remainder in `requiredFutureTaskNodes` without reopening any implemented closeout.

Remaining nodes, in task-graph order: `organization-rbac-menu-contract-and-migration-design`, `organization-role-pool-backend-and-migration`, `organization-user-admin-experience`, `role-tree-and-authorization-entry`, `menu-tree-and-button-permission-configuration`, `organization-rbac-menu-e2e-qa`, `multi-datasource-contract-and-runtime`, `tenant-placement-and-request-routing`, `datasource-read-write-routing`, `sharding-and-tenant-migration`, `federated-read-query`, `xa-optional-adapter`, `database-certification-matrix`, `transactional-outbox-and-one-mq-adapter`, `asynchronous-search-projection`, `open-source-portability`, `public-docs-community`, `public-docs-site`, `github-release-publication`.

The 19 unfinished nodes remain governed by the [activated remaining-task topology](superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md). The service contract, persisted Query/Command runtime and disabled integration ports are closed; the organization contract lane is now unblocked while datasource work retains its independent dependencies and locks.
...
3. Generator hardening
   ...
   - Current state: `resources/platform-task-execution-audit.json` records the exact ordered 19-node remainder, cross-lane dependencies, domain locks and approved parallel windows. Runtime/source-writing promotion blockers remain explicit; operational UI work stays tied to Product Design and browser evidence.
   - Current state: `resources/platform-goal-completion-audit.json` records `not-complete-controlled` at 66 total, 47 implemented and 19 unfinished. `resources/platform-node-closeout-audit.json` preserves 47 implemented closeouts and rejects closeout evidence for unfinished nodes. `resources/platform-objective-conformance.json` projects the same 19 controlled blockers.
...
The original foundation baseline and ten completed continuation nodes remain closed. The active graph is `66 total / 47 implemented / 19 controlled unfinished`; external production promotion remains `not-approved`, and runtime mutation, refresh-token-family default runtime, source writing, uncertified databases, federation mutation, default XA and messaging/search adapters remain disabled.
