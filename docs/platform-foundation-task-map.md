...
The 2026-07-14 topology activation supersedes only the old organization/RBAC/menu target semantics; it does not add a second organization model. `org-units`, tenants, users, roles, role groups and area codes remain non-droppable primitives. The six organization nodes must first design and migrate `org_unit_role_groups`, single-group role ownership, tenant derivation, `role_menu`, directory/page menus and separate page-button/API permissions, then deliver the organization/user UI, strict role tree, menu tree and full browser QA. Personnel files, positions and assignments remain optional capability resources and continue to reuse the shared tenant and organization boundary.
...
The P1/P2 foundation items below are represented in `resources/platform-foundation-task-graph.json`. The original 37-node foundation baseline and thirteen completed continuation nodes remain closed, producing `66 total / 50 implemented / 16 controlled unfinished`; `resources/platform-foundation-alignment-audit.json` keeps the exact 16-node remainder in `requiredFutureTaskNodes` without reopening any implemented closeout.

Remaining nodes, in task-graph order: `role-tree-and-authorization-entry`, `menu-tree-and-button-permission-configuration`, `organization-rbac-menu-e2e-qa`, `multi-datasource-contract-and-runtime`, `tenant-placement-and-request-routing`, `datasource-read-write-routing`, `sharding-and-tenant-migration`, `federated-read-query`, `xa-optional-adapter`, `database-certification-matrix`, `transactional-outbox-and-one-mq-adapter`, `asynchronous-search-projection`, `open-source-portability`, `public-docs-community`, `public-docs-site`, `github-release-publication`.

The 16 unfinished nodes remain governed by the [activated remaining-task topology](superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md). The service contract, persisted Query/Command runtime, disabled integration ports, [organization/RBAC/menu contract](platform-organization-rbac-menu-contract.md), organization role-pool backend/migration runtime and organization/user Admin experience are closed; `role-tree-and-authorization-entry` is the next organization-lane node while datasource work retains its independent dependencies and locks.
...
3. Generator hardening
   ...
   - Current state: `resources/platform-task-execution-audit.json` records the exact ordered 16-node remainder, cross-lane dependencies, domain locks and approved parallel windows. Runtime/source-writing promotion blockers remain explicit; operational UI work stays tied to Product Design and browser evidence.
   - Current state: `resources/platform-goal-completion-audit.json` records `not-complete-controlled` at 66 total, 50 implemented and 16 unfinished. `resources/platform-node-closeout-audit.json` preserves 50 implemented closeouts and rejects closeout evidence for unfinished nodes. `resources/platform-objective-conformance.json` projects the same 16 controlled blockers.
...
The original foundation baseline and thirteen completed continuation nodes remain closed. The active graph is `66 total / 50 implemented / 16 controlled unfinished`; external production promotion remains `not-approved`, and runtime mutation, refresh-token-family default runtime, source writing, uncertified databases, federation mutation, default XA and messaging/search adapters remain disabled.
