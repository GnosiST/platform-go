...
1. Production auth hardening
   ...
   - Current state: the Provider Promotion Matrix now classifies built-in providers before promotion. `demo` stays local-harness-only; `wechat` is optional for production and requires config-key, credential-owner, rotation-runbook, subject-redaction, unconfigured-provider rejection and source-backed runtime evidence before use.
...
3. Generator hardening
   ...
   - Current state: `resources/platform-task-execution-audit.json` records zero unfinished nodes, future runtime/source-writing promotion blockers, required validators and conflict-free parallel batches. `scripts/validate-platform-task-execution-audit.mjs` requires submitted promotion evidence package validation and keeps visual/UI work tied to both `superpowers:brainstorming` and `product-design` evidence.
   - Current state: `resources/platform-goal-completion-audit.json` records the active foundation goal as `complete`, and `scripts/validate-platform-goal-completion-audit.mjs` rejects completion claims that drift from business-neutral reference boundaries, approved stack evidence, task counts, disabled refresh-token-family default runtime, disabled source-writing mode or external artifact URI requirements for future runtime mutation. `resources/platform-node-closeout-audit.json` records closeout evidence for implemented nodes and rejects missing neat-freak cleanup evidence or visual closeout without both `superpowers:brainstorming` and product-design evidence. `resources/platform-objective-conformance.json` is the persistent objective gate: `scripts/validate-platform-objective-conformance.mjs` keeps approved stack, reference-only `zshenmez` usage, capability-contract governance, future promotion gates, visual gate order, Vercel boundary, production preflight and node closeout cleanup aligned before further implementation continues.
