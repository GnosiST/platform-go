...
The P1/P2 foundation items below are represented in `resources/platform-foundation-task-graph.json`. The original 37-node foundation baseline remains implemented and closed, including Task 8 production Admin OIDC evidence. `runtime-security-containment`, `admin-watermark-export-governance`, `sensitive-data-protection-runtime` and `sensitive-data-historical-migration` are `implemented`, producing `53 total / 41 implemented / 12 controlled unfinished`; `resources/platform-foundation-alignment-audit.json` keeps the twelve remaining nodes in `requiredFutureTaskNodes` while keeping both implemented sensitive-data nodes in the required task set.

Remaining nodes, in task-graph order: `mask-strategy-runtime`, `sensitive-data-reveal-step-up`, `data-lifecycle-retention`, `multi-datasource-contract-and-runtime`, `database-certification-matrix`, `integration-ports-disabled-default`, `transactional-outbox-and-one-mq-adapter`, `asynchronous-search-projection`, `open-source-portability`, `public-docs-community`, `public-docs-site`, `github-release-publication`.
...
3. Generator hardening
   ...
   - Current state: `resources/platform-task-execution-audit.json` records the ordered twelve-node remainder of the completion program as controlled unfinished work. Future runtime/source-writing promotion blockers, required validators and conflict-free parallel batches remain explicit; visual/UI work stays tied to both `superpowers:brainstorming` and `product-design` evidence.
   - Current state: `resources/platform-goal-completion-audit.json` records `not-complete-controlled` at 53 total, 41 implemented and 12 unfinished. `resources/platform-node-closeout-audit.json` records 41 implemented closeouts and rejects closeout evidence for pending nodes. `resources/platform-objective-conformance.json` projects the same twelve controlled blockers while keeping approved stack, reference-only `zshenmez` usage, capability-contract governance, future promotion gates, visual gate order, Vercel boundary and production preflight aligned.
