...
The original foundation baseline and completed continuation nodes remain closed. The active graph is `67 total / 58 implemented / 9 controlled unfinished`; external production promotion remains `not-approved`, and runtime mutation, refresh-token-family default runtime, source writing, uncertified databases, federation mutation, default XA and messaging/search adapters remain disabled.
