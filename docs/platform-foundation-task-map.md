...
The P1/P2 foundation items below are represented in `resources/platform-foundation-task-graph.json`. The original 37-node foundation baseline remains implemented and closed, including Task 8 production Admin OIDC evidence. The active completion program adds eight controlled pending nodes, producing `45 total / 37 implemented / 8 controlled unfinished`; `resources/platform-foundation-alignment-audit.json` projects those eight nodes through `requiredFutureTaskNodes` without moving the preserved baseline out of `requiredTaskNodes`.

The approved program is defined by `docs/superpowers/specs/2026-07-12-platform-go-completion-program-design.md`, `docs/superpowers/specs/2026-07-12-runtime-security-hardening-design.md`, `docs/superpowers/specs/2026-07-12-admin-watermark-export-design.md`, `docs/superpowers/specs/2026-07-12-sensitive-data-encryption-design.md` and `docs/superpowers/specs/2026-07-12-open-source-docs-site-design.md`.
...
3. Generator hardening
   ...
   - Current state: `resources/platform-task-execution-audit.json` records the ordered eight-node completion program as controlled unfinished work. Future runtime/source-writing promotion blockers, required validators and conflict-free parallel batches remain explicit; visual/UI work stays tied to both `superpowers:brainstorming` and `product-design` evidence.
   - Current state: `resources/platform-goal-completion-audit.json` records `not-complete-controlled` at 45 total, 37 implemented and 8 unfinished. `resources/platform-node-closeout-audit.json` preserves the original 37 closeouts and rejects closeout evidence for pending nodes. `resources/platform-objective-conformance.json` projects the same eight controlled blockers while keeping approved stack, reference-only `zshenmez` usage, capability-contract governance, future promotion gates, visual gate order, Vercel boundary and production preflight aligned.
...
The original foundation baseline remains closed at 37 implemented nodes. The active completion program expands the graph to 45 nodes with eight controlled pending tasks for runtime security, watermark/export governance, sensitive-data runtime and migration, open-source portability, public community/docs work and GitHub publication. External production promotion remains `not-approved`; runtime mutation, refresh-token-family default runtime and source writing remain disabled.
