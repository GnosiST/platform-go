...
Page buttons are edited inside the selected page detail, not as a separate top-level Admin resource. Each row has a stable `buttonKey`, localized label, action, sort order, status and exactly one `page-button` permission code whose metadata points back to the same menu and button. `platform.navigation.menu-definition.get` returns the complete menu/button definition and revision. `platform.navigation.menu-definition.create` requires menu-create permission, while `platform.navigation.menu-definition.replace` requires menu-update permission; both use the same native transaction to create or update the menu, buttons and managed page-button permission relations. The Admin must not compose this operation from generic menu and permission CRUD calls. Deletion remains on the existing reviewed lifecycle impact/apply path. Button visibility never authorizes an API; the UI shows a coherence warning when the role lacks useful API permissions but never grants or hides access implicitly.
