...
- The original foundation baseline is closed at 37 implemented nodes. Fourteen continuation nodes, including `platform-service-contract-standard`, `persisted-query-command-object-runtime`, `integration-ports-disabled-default`, the organization/RBAC/menu contract gate, the organization role-pool backend/migration runtime, `organization-user-admin-experience` and `role-tree-and-authorization-entry`, are implemented, so the active completion program is now `66 total / 51 implemented / 15 controlled unfinished` and the current goal remains `not-complete-controlled`.
...
- The exact unfinished order starts with `menu-tree-and-button-permission-configuration`, then the full organization E2E and datasource/runtime lanes, followed by database certification, Outbox/MQ, search projection and the four release nodes. The [activated remaining-task topology](superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md) remains the authority; earlier fragmented static-datasource, SQL-less, Query Object and partial service-contract drafts are superseded.
...
- Governance: the 37-node foundation baseline remains closed, fourteen continuation nodes remain `implemented`, and the graph is `66 total / 51 implemented / 15 controlled unfinished` with status `not-complete-controlled`. The topology activation does not reopen any closeout. Production promotion remains `not-approved`; runtime mutation and refresh-token-family default runtime remain disabled.
