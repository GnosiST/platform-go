...
- Done, completion program: `admin-watermark-export-governance` is `implemented`. The normalized settings contract preserves legacy boolean configuration, supports independent screen/export scopes, renders exactly `1/4/9/16` inert screen marks, and adds structured provenance only to Policy Review JSON exports. Product Design, `ui-ux-pro-max`, bilingual contract/build checks, desktop and `390x844` browser evidence, dark-mode coverage and a clean console are recorded in the task graph and node closeout audit.
