...
- The persistent full-scope unfinished inventory starts with `organization-rbac-menu-e2e-qa` and `unified-error-code-governance`, contains the nine post-release optional deferred datasource/runtime nodes in task-graph order, then ends with `open-source-portability`, public docs and publication. The v0.1.0 execution path omits the deferred lane: organization E2E and unified error-code governance close first, followed by portability, public docs/site and publication. The [activated remaining-task topology](superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md) remains the authority; release eligibility is separate from persistent full-goal completion.
