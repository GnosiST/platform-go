...
- Notification: the optional `notification` capability now declares the `message-center` workbench route plus `notification-channels`, `notification-providers`, `notification-send-policies`, `notification-templates`, `notifications` and `notification-deliveries`. Channel contracts cover in-app, SMS, email, WeChat official account and WeChat miniapp; SMS v1 has Aliyun/Tencent/mock-local provider configuration, official SDK-backed Aliyun/Tencent live SMS adapters, dry-run/config validation, vendor error normalization, the message delivery worker contract, manual run endpoint `POST /api/admin/message-center/deliveries/run` and `/settings` configuration-page closure. SMTP/WeChat send adapters and real-vendor production delivery evidence remain follow-up runtime/promotion slices.
