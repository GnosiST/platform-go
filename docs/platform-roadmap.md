...
`github-release-publication` is `pending` and remains the sole v0.1.0 release blocker. v0.1.0 publication is explicitly authorized; the repository, CI and Pages are ready, and the node closes after the formal Git tag, GitHub Release and publication evidence are verified.
...
- The persistent full-scope unfinished inventory contains the nine post-release optional deferred datasource/runtime nodes in task-graph order plus the pending `github-release-publication` gate. `open-source-portability`, `public-docs-community` and `public-docs-site` are `implemented`; v0.1.0 publication is authorized and awaits the formal tag, Release and evidence capture.
...
- Governance: the 37-node foundation baseline and completed continuation nodes remain closed, and the graph is `67 total / 57 implemented / 10 controlled unfinished` with status `not-complete-controlled`. Open-source portability, public docs/community and the documentation site are implemented; publication governance is `pending`, and the nine post-release optional nodes remain `deferred`. Production promotion remains `not-approved`; target menu serving, role-menu migration writes, runtime mutation and refresh-token-family default runtime remain disabled.
