...
- Notification: the optional `notification` capability now declares the `message-center` workbench route plus `notification-channels`, `notification-providers`, `notification-send-policies`, `notification-templates`, `notifications` and `notification-deliveries`. Channel contracts cover in-app, SMS, email, WeChat official account and WeChat miniapp; SMS P0 has Aliyun/Tencent/mock-local configuration entries, email P0 has generic SMTP, and WeChat channels have configuration placeholders. Real provider adapters, vendor error normalization and retry workers remain follow-up runtime slices.
