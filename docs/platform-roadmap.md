...
- optional `notification` capability with a `message-center` workbench entry, channel configuration, provider accounts, send policies, `notification-templates`, `notifications`, `notification-deliveries` and shared tenant/user/audit relations through the `platform-notification-ready` profile;
...
- Keep topbar interface preferences in `SystemSettingsDrawer`, including theme, layout, density, work tabs, sidebar collapse, watermark, visual aid, page transition, import/export and reset; system-level function configuration belongs to the `/settings` workbench and enabled capability configuration resources.
...
- Notification: the optional `notification` capability now declares the `message-center` workbench route plus `notification-channels`, `notification-providers`, `notification-send-policies`, `notification-templates`, `notifications` and `notification-deliveries`. Channel contracts cover in-app, SMS, email, WeChat official account and WeChat miniapp; SMS v1 has Aliyun/Tencent/mock-local provider configuration, official SDK-backed Aliyun/Tencent live SMS adapters, dry-run/config validation, vendor error normalization, the message delivery worker contract, manual run endpoint `POST /api/admin/message-center/deliveries/run`, local dry-run worker receipts for in-app/email/WeChat channels, safe retry endpoint `POST /api/admin/message-center/deliveries/{id}/retry`, failed-attempt backoff ledger fields and `/settings` configuration-page closure. Email and WeChat test-send/test-connect/worker currently prove local dry-run workbench and delivery-ledger closure only; SMTP/WeChat external send adapters and real-vendor production delivery evidence remain follow-up runtime/promotion slices.
- Settings runtime: `/settings` v1.1 now dynamically aggregates enabled capability configuration resources, including notification channel/provider/template/policy records and `credential-auth-settings` when those capabilities are enabled. Settings updates audit `settings.update`, allow only public/plain fields plus write-only encrypted secret fields, omit protected values from responses, and return `restartRequired/pendingRestart`; validate-config and test-connect endpoints perform local schema/adapter preflight without requiring real vendor accounts.
