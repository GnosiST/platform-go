...
- The original foundation baseline is closed at 37 implemented nodes. `runtime-security-containment`, `admin-watermark-export-governance`, `sensitive-data-protection-runtime`, `sensitive-data-historical-migration`, `mask-strategy-runtime`, `sensitive-data-reveal-step-up` and `data-lifecycle-retention` are `implemented`, so the active completion program is now `53 total / 44 implemented / 9 controlled unfinished` and the current goal remains `not-complete-controlled`.
- The currently effective five data/integration nodes and four open-source nodes remain pending until the separate task-graph activation commit. The only approved expansion and ordering is the [remaining-task topology adjustment](superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md), which projects `66 total / 44 implemented / 22 controlled unfinished` after activation; earlier fragmented static-datasource, SQL-less, Query Object and partial service-contract drafts are superseded.
...
- Historical migration is maintenance-only with seven CLI modes, manifest-derived `Source="values"` targeting, prepare-only journal creation, bounded tenant cursors, encrypted escrow, restore rehearsal and hash-guarded rollback; it adds no HTTP route or plaintext dual-read. Lifecycle retention is maintenance-only for final purge, requires completed dry-run plus exact promotion evidence, and keeps its scheduler disabled by default. The nine pending nodes have no implementation closeout. MySQL/PostgreSQL promotion still requires real integration certification evidence, SQLite remains local development/test rehearsal only, Oracle and KingbaseES remain uncertified, and messaging/search integrations remain disabled by default. The approved specifications cover the [program](superpowers/specs/2026-07-12-platform-go-completion-program-design.md), [runtime security](superpowers/specs/2026-07-12-runtime-security-hardening-design.md), [watermark/export governance](superpowers/specs/2026-07-12-admin-watermark-export-design.md), [sensitive data encryption](superpowers/specs/2026-07-12-sensitive-data-encryption-design.md), [historical migration](superpowers/specs/2026-07-12-sensitive-data-historical-migration-design.md), [mask strategy runtime](superpowers/specs/2026-07-13-mask-strategy-runtime-design.md), [sensitive data reveal step-up](superpowers/specs/2026-07-13-sensitive-data-reveal-step-up-design.md), [data lifecycle retention](superpowers/specs/2026-07-14-data-lifecycle-retention-design.md) and [open-source docs/site](superpowers/specs/2026-07-12-open-source-docs-site-design.md). Production-auth and source-writing promotion remain intentionally non-mutating and `not-approved`; refresh-token-family default runtime and real source writing remain disabled. The platform remains business-neutral and does not ship `zshenmez` business packages, cutovers, state machines or business stores.
...
- Governance: the 37-node foundation baseline remains closed, `runtime-security-containment`, `admin-watermark-export-governance`, `sensitive-data-protection-runtime`, `sensitive-data-historical-migration`, `mask-strategy-runtime`, `sensitive-data-reveal-step-up` and `data-lifecycle-retention` are `implemented`, and the effective graph remains `53 total / 44 implemented / 9 controlled unfinished` and `not-complete-controlled` until activation. The approved topology proposal projects `66 / 44 / 22` without reopening those closeouts. `production-admin-oidc-auth` retains source, automated, Product Design, `ui-ux-pro-max`, accessibility, production-like Keycloak, six-viewport browser and phase-closeout neat-freak evidence. Production promotion remains `not-approved`; runtime mutation and refresh-token-family default runtime remain disabled.
