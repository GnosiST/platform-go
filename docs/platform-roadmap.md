...
- Governance: the 37-node foundation baseline remains closed, `runtime-security-containment`, `admin-watermark-export-governance`, `sensitive-data-protection-runtime` and `sensitive-data-historical-migration` are `implemented`, and the active completion program keeps the full graph at `45 total / 41 implemented / 4 controlled unfinished` and `not-complete-controlled`. `production-admin-oidc-auth` retains source, automated, Product Design, `ui-ux-pro-max`, accessibility, production-like Keycloak, six-viewport browser and neat-freak closeout evidence. Production promotion remains `not-approved`; runtime mutation and refresh-token-family default runtime remain disabled.
