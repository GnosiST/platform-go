...
- node closeout audit in `resources/platform-node-closeout-audit.json`, with a validator that requires every implemented task node to carry neat-freak cleanup evidence, docs/tests/validator evidence, resource-lock review, objective-conflict review and visual design-gate evidence where required;
...
- Governance: the 37-node foundation baseline remains closed, `runtime-security-containment`, `admin-watermark-export-governance`, `sensitive-data-protection-runtime` and `sensitive-data-historical-migration` are `implemented`, and the active completion program keeps the full graph at `53 total / 41 implemented / 12 controlled unfinished` and `not-complete-controlled`. The eight newly governed data/integration nodes and four existing open-source nodes listed above remain pending. `production-admin-oidc-auth` retains source, automated, Product Design, `ui-ux-pro-max`, accessibility, production-like Keycloak, six-viewport browser and neat-freak closeout evidence. Production promotion remains `not-approved`; runtime mutation and refresh-token-family default runtime remain disabled.
