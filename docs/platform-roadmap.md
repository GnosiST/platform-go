...
- The original foundation baseline is closed at 37 implemented nodes. Nine continuation nodes, including `platform-service-contract-standard` and `integration-ports-disabled-default`, are implemented, so the active completion program is now `66 total / 46 implemented / 20 controlled unfinished` and the current goal remains `not-complete-controlled`.
- The exact unfinished order starts with `persisted-query-command-object-runtime` and `integration-ports-disabled-default`, then the organization/RBAC/menu lane and datasource/runtime lane, followed by database certification, Outbox/MQ, search projection and the four release nodes. The [activated remaining-task topology](superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md) remains the authority; earlier fragmented static-datasource, SQL-less, Query Object and partial service-contract drafts are superseded.
...
- Governance: the 37-node foundation baseline remains closed, nine continuation nodes remain `implemented`, and the graph is `66 total / 46 implemented / 20 controlled unfinished` with status `not-complete-controlled`. The topology activation does not reopen any closeout. Production promotion remains `not-approved`; runtime mutation and refresh-token-family default runtime remain disabled.
