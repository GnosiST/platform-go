...
- Governance: the foundation graph is closed at 37 total / 37 implemented / 0 pending / 0 blocked. `production-admin-oidc-auth` includes source, automated, Product Design, `ui-ux-pro-max`, accessibility, production-like Keycloak, six-viewport browser and neat-freak closeout evidence. Production promotion remains `not-approved`; runtime mutation and refresh-token-family remain disabled.
