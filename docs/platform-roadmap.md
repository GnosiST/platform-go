...
- The exact unfinished order starts with `organization-rbac-menu-e2e-qa` and `unified-error-code-governance`, then the nine post-release optional deferred datasource/runtime nodes, followed by `open-source-portability`, public docs and publication. The [activated remaining-task topology](superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md) remains the authority; release eligibility is separate from persistent full-goal completion.
