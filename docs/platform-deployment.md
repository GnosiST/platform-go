...
The standard adapter is an origin behind a reviewed external TLS edge. It binds the Admin proxy to loopback by default, installs `platform.conf` through the official Nginx `/etc/nginx/templates` envsubst entrypoint, redirects to `PLATFORM_PUBLIC_BASE_URL` rather than the request Host, and emits HSTS only after the reviewed edge supplies one canonical `https` signal. Configure `PLATFORM_TRUSTED_PROXIES` so `PLATFORM_ADMIN_PROXY_IP` is contained by the API policy. Configure `PLATFORM_EDGE_TRUSTED_PROXY` as one canonical direct edge peer IP inside `PLATFORM_INTERNAL_SUBNET`; CIDRs, loopback, unspecified and multicast addresses are rejected. Nginx accepts real IP and forwarded protocol only from that peer, then overwrites `X-Forwarded-For` instead of appending caller-controlled chain state. Reject API trusted-proxy policies that cumulatively cover all IPv4 or IPv6 addresses, and set a positive bounded `PLATFORM_HTTP_MAX_BODY_BYTES`. The API container healthcheck is the sole HTTP exception: direct loopback `GET /api/ready`, which verifies configured runtime dependencies without exposing their details and emits no HSTS. `GET /api/health` remains the process liveness endpoint. Never expose port 8080 as an unreviewed public HTTP origin.

### Observability Baseline

The foundation emits W3C `traceparent` correlation on API responses and records safe request, login, error and delivery audit data through the configured Admin-resource store. Use the returned request and trace IDs to correlate application events; request payloads, credentials, tokens and protected values are excluded. The protected `GET /api/platform/cache/stats` endpoint requires `admin:monitoring:read` and exposes cache hit, miss, write, delete and error counters. These are the current built-in operational signals. OpenTelemetry export, a metrics scrape endpoint and a tracing collector are not configured by default; add them only through a reviewed optional integration with explicit exporter, retention and sensitive-data rules.
...
Credential authentication is also optional. When any `PLATFORM_CREDENTIAL_AUTH_*` provider switch enables it in production, the production environment gate requires the `credential-auth` capability, a dedicated repository driver/DSN, a 32-byte-or-longer identifier HMAC key, and a valid P-256 secret-transport private key with its key ID. Password, phone and email provider modes also require their matching bootstrap administrator fields; phone SMS OTP additionally requires `notification`, a non-debug SMS provider and the login template ID. The stock template intentionally leaves these values absent; use `docs/platform-auth.md` for the complete private configuration list.
