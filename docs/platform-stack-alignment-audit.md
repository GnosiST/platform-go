...
| JWT auth | Present | Admin HTTP auth returns JWT bearer tokens with session/revocation checks plus sliding renewal; app HTTP auth has an isolated `tokenType=app` runtime | Add independent refresh-token rotation, business app identity binding and distributed revocation policies when production auth requires them. |
...
6. **Replace opaque sessions with JWT sessions.** Admin HTTP, admin sliding renewal and isolated app HTTP slices done.
   Keep server-side session revocation, repository-backed convergence and cache invalidation behavior. Independent refresh-token-family default binding remains blocked by the production approval gate; product-specific identity adoption and additional provider adapters belong downstream.
...
10. **Rebuild optional capabilities.** Base slices done.
    WeChat login, branding, demo data, Redis cache and file storage sit on the corrected stack; remaining work is production adapter hardening and business adoption.
