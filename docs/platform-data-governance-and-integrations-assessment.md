...
It does not mark every assessed capability as implemented. Governance now records `66 total / 49 implemented / 17 controlled unfinished`; `mask-strategy-runtime`, `sensitive-data-reveal-step-up`, `data-lifecycle-retention`, `platform-service-contract-standard`, `persisted-query-command-object-runtime`, the disabled-by-default integration ports, the organization/RBAC/menu contract gate and the organization role-pool backend/migration runtime are implemented and closed. Organization Admin/menu UI and E2E, datasource routing, database certification, concrete MQ/search adapters and publication work remain pending.

The [remaining-task topology adjustment](superpowers/specs/2026-07-14-platform-remaining-task-topology-adjustment.md) remains the activated source of truth for the program boundaries and dependencies. The service contract, persisted Query/Command runtime, disabled integration ports and organization contract are closed; the remaining 18 nodes retain their approved order and independent completion gates.
