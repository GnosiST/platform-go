...
| Passwords and transport | The default platform has no legacy `password` provider kind. The optional `credential-auth` P0 runtime uses dedicated Argon2id credential storage plus application-layer hybrid encryption: clients fetch `GET /api/auth/credential-secret-key`, use ECDH P-256, HKDF-SHA256 and AES-256-GCM/A256GCM, then submit `secret.encrypted` for password or SMS OTP secrets. HTTPS and HSTS remain the production baseline. | HTTPS cannot be used as a substitute for `secret.encrypted`; plaintext `secret.value` or `secret.code` must be rejected when encrypted secrets are required. The capability is still not production-complete and must never use generic resource values or reversible password encryption. |
