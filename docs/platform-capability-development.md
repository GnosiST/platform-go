...
Delete first saves an internal `deletionState=pending` tombstone, `deletionRequestedAt` and `file.delete.request` audit atomically. Tombstoned records are immediately hidden from list/query/content projections while retaining the internal object key for recovery. Object deletion is idempotently retryable; `storage.ErrObjectNotFound` is treated as already clean, while other failures leave the tombstone hidden for a later retry. Only successful object cleanup is followed by atomic metadata purge and `file.delete` completion audit. This ordering prevents a visible record from pointing at a missing object and prevents an audit failure from restoring access or losing the only recoverable object reference.
