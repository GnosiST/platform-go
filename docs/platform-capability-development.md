...
When the goal is to start a real business project from the reusable base, use a downstream business package or service rather than editing platform source. The maintained runnable slice is `examples/external-capability`, and the step-by-step guide is `docs/external-business-project-template.md`.

The template proves the minimum onboarding contract without external configuration:
...
Because the example is intentionally a standalone nested Go module, root-module testing must use `go -C examples/external-capability` or run from that directory. The shape mirrors a downstream repository: pin a reviewed `platform-go` Git commit SHA, import only `github.com/GnosiST/platform-go/pkg/platform/capability`, export a business manifest, register it in the downstream composition root, regenerate contracts for the enabled capability set, rebuild the downstream artifact and restart. The published `v0.1.0` tag is immutable; a later mainline closure that keeps package version `0.1.0` must be consumed by its reviewed SHA rather than by moving that tag.

The sample `example-catalog` domain is replaceable tutorial material only. Business resources such as product-specific workflows, exhibitors, orders, portfolios or support tickets must stay in downstream capability manifests until they are proven reusable platform capabilities with their own public contract, profile policy, validators, docs and migration evidence.
...
Provider declarations may be enabled but unconfigured. Login selectors must render only providers that are both enabled and configured; unconfigured providers remain visible through capability and settings governance instead of appearing as disabled login choices.
...
The current platform intentionally has no legacy local-password provider kind wired into HTTP login. The API composition root rejects the explicit provider kind `password`; provider IDs and generic field names do not select authentication semantics. `credential-auth` owns the separate Argon2id service foundation and uses explicit `credential-password` / `credential-sms-otp` provider kinds for its deliverable-v1 runtime. Its reviewed rotation/incident/migration governance contract requires rehash-on-success, `MustChange` on privileged reset, dedicated credential tables and external approval evidence before production promotion; it must not persist password hashes in generic `Record.Values` or make hashing policy a mutable project-initialization switch.

### credential-auth capability rules

`credential-auth` is the local credential authentication capability for username/password, phone/password, email/password and phone/SMS OTP login. The current package is `deliverable-v1`: `resources/platform-credential-auth-v1.json`, `scripts/validate-platform-credential-auth-v1.mjs`, `scripts/platform-credential-auth-v1.test.mjs`, `internal/platform/credentialauth`, `internal/platform/httpapi` and `admin/src/platform/auth` define the boundary, provider declarations, internal service foundation, dedicated GORM credential repository, application-layer hybrid encrypted secret transport, HTTP login slice, `POST /api/auth/challenges`, password change/reset contract and provider-driven Admin login UI. It is not production-complete.

Do not declare provider kind `password` in a manifest. The current `cmd/platform-api` guard must keep rejecting `password` provider kind; credential-auth provider declarations use the explicit `credential-password` and `credential-sms-otp` kinds. Existing demo, Admin OIDC and app provider login flows must remain compatible while the credential-auth API contract is `deliverable-v1`.

Do not store password hashes, raw passwords, OTP codes, challenge answers or credential reset material in generic `Record.Values`. `credential-auth` must use dedicated stores for `auth_identifiers`, `password_credentials`, `credential_challenges` and `sms_otp_challenges`; generic resources may only expose masked or verified identifier metadata when the owning capability explicitly declares sensitivity, storage and projection rules. Password verification uses Argon2id with a versioned parameter policy and rehash-on-upgrade behavior. Browser clients must not submit local passwords or OTP codes as plaintext JSON; they fetch `GET /api/auth/credential-secret-key`, use ECDH P-256 to agree on a shared key, derive with HKDF-SHA256, encrypt the password or SMS OTP secret with AES-256-GCM/A256GCM, and submit an ECDH-P256-HKDF-SHA256+A256GCM `secret.encrypted` envelope to `POST /api/auth/login`. The password change/reset contract uses the same encrypted envelope on `POST /api/admin/profile/current/password/change` and `POST /api/admin/profile/{id}/password/reset`. When encrypted secrets are required, `secret.value` and `secret.code` are invalid even if `secret.encrypted` is also present. HTTPS remains required in production, but it is not the application-layer credential secret encryption mechanism and cannot be used as a substitute for `secret.encrypted`.

`POST /api/auth/challenges` is now a backend runtime endpoint for login challenge transactions. It creates digest-only CAPTCHA/slider state, applies rate-limit enforcement and keeps raw challenge proofs and client fingerprints out of generic resource values. Credential-auth v1 requires its valid single-use challenge for password login, SMS OTP send and SMS OTP login; malformed slider proofs consume challenge attempts instead of bypassing the attempt counter. Challenge and SMS OTP records are disabled on max-attempt rejection, so later correct proofs cannot revive an exhausted transaction. The Admin login UI renders text CAPTCHA and go-captcha slider material, submits the same per-page challenge nonce on start, SMS OTP send and login, and the backend accepts a small digest-only slider X tolerance without persisting raw coordinates. Risk-based routing remains a separate reviewed capability because no raw client identity may be retained to implement it.

SMS OTP is an authentication secret type owned by `credential-auth`, but SMS delivery is an extension of the reusable `notification` capability. SMS OTP start uses an IP/provider coarse rate limit before identifier resolution, requires a valid single-use credential challenge before sending, persists the digest-only OTP transaction before calling the SMS sender, disables that transaction if sending fails, and keeps the existing normalized phoneHash limit before delivery; neither key may include raw phone, OTP or credential material. Credential login failures write redacted `auth.login` failure audit records by provider dimension only. The current notification SMS foundation provides the sender port, `mock-local` development/test sender, provider canonicalization and config validation that rejects mock SMS outside development/test. Aliyun/Tencent can run as non-network dry-run senders for configuration validation or as official SDK-backed live senders when `PLATFORM_NOTIFICATION_SMS_LIVE_SEND_ENABLED=true`; vendor errors are normalized before they reach delivery ledgers or public responses. Channel-level limits and sender selection belong to `notification`, not to a business package or the Admin shell. Production credential-auth requires the dedicated repository, secret transport key configuration and real-vendor delivery evidence before promotion.

`credential-auth` declares the `credential-auth-settings` Admin resource under the configuration/security group so `/settings` can dynamically expose password-provider toggles, challenge mode/kind, password failure limits, lock duration, SMS OTP TTL/attempt policy, `argon2id` parameter version and application-layer secret transport policy. This resource is configuration metadata only: it must not contain password hashes, raw passwords, OTP codes, challenge answers or credential reset material. Changes are saved through `PUT /api/admin/settings/{resource}/{id}`, audited as `settings.update`, and marked `restartRequired/pendingRestart` because credential providers and security policy are process-runtime inputs in v1.1.

The productized `notification` contract includes a read-only/custom `message-center` workbench entry at `/message-center` with permission prefix `admin:message-center`; it is an Admin UI composition point and does not replace the lower-level CRUD contracts. The same capability declares `notification-channels`, `notification-providers` and `notification-send-policies` alongside `notification-templates`, `notifications` and `notification-deliveries`. Channel options cover `in_app`, `sms`, `email`, `wechat_official` and `wechat_miniapp`; provider options cover SMS P0 `aliyun`, `tencent` and development/test-only `mock-local`, email P0 generic `smtp`, plus WeChat official account and miniapp configuration placeholders. Email and WeChat test-send flows may use local dry-run receipts to prove the Admin workbench, template and delivery-ledger loop, but they are not external supplier integrations. Provider credential fields must be explicitly marked `secret` or `sensitive` with encrypted, masked or omitted projections; do not store provider keys, app secrets, webhook secrets or raw vendor errors as plain generic `values`.

Message center v1 includes SMS provider configuration, Aliyun/Tencent live SMS SDK adapters, dry-run validation, local dry-run receipts for in-app/email/WeChat test-send and delivery-worker flows, the message delivery worker contract, manual run endpoint `POST /api/admin/message-center/deliveries/run`, safe retry endpoint `POST /api/admin/message-center/deliveries/{id}/retry` and configuration page closure through `/settings`. A retry recipient is a write-only process-memory lease, retained for transient retries and released after successful ledger mutation; raw targets never enter generic delivery values, and restart/expiry requires an authorized retry with a new recipient. The current provider truth is SMS live SDK adapters plus dry-run/config validation; in-app, Email and WeChat worker paths are local dry-run ledger paths only. Do not claim SMTP, WeChat official account or WeChat miniapp supplier integration until separate adapter SDK integration and redacted evidence packages exist.

Notification send policy limits are runtime-active in v1: `notification-send-policies.rateLimitPerMinute`, `notification-channels.rateLimitPerMinute` and `notification-channels.dailyQuota` are read from enabled configuration records and enforced through `ratelimit.OperationMessageCenterDelivery` before message-center test sends and delivery-worker attempts. The rate-limit keys use tenant, channel, provider, template, purpose, actor and client dimensions; they must not include raw recipients, OTP codes or template parameter values. Failed worker attempts write sanitized error state, retry backoff seconds and next retry time into the delivery ledger. A manual retry requeues a failed delivery while keeping the stored target redacted; the supplied recipient is held only in the bounded write-only runtime lease until successful delivery or expiry and must not be written into generic delivery values. Quiet-hour scheduling and per-supplier quota reconciliation remain follow-up runtime slices.

`resources/platform-capability-contracts.json` records this as `notification.productization`, and `scripts/validate-platform-capability-contracts.mjs` treats it as a product gate. A change that leaves only SMS, removes generic SMTP, drops WeChat official account or miniapp configuration placeholders, or bypasses `/settings` dynamic aggregation must fail validation. The product rule is: `/message-center` is the operational workbench, `/settings` is the system configuration center for enabled notification configuration resources, Aliyun/Tencent SMS can run through official SDK adapters, Email/WeChat can only claim local dry-run workbench closure, and concrete SMTP/WeChat sending adapters remain runtime follow-up slices until their provider SDKs, credential storage, retry/error normalization and production evidence are implemented.

Admin login UI must use `GET /api/auth/providers` discovery and render only enabled, configured provider modes from the returned declarations. Do not hard-code the four credential modes into shell layout logic; provider-specific state machines belong behind the capability contract and shared auth UI components. The current UI slice covers credential password, phone SMS OTP, text CAPTCHA and go-captcha slider challenge flows for both SMS OTP send and credential login; production risk scoring and broader challenge policy hardening remain future work.

Verify the contract before changing local credential auth files:
...
### System settings center rules

`/settings` is the product entry for system-level function configuration. It is owned by the `parameter` capability through the read-only `settings` workbench resource, and it dynamically groups enabled capability configuration resources instead of adding one permanent navigation item per capability. The topbar settings button is not this configuration center; it is an interface-preference surface for theme, density, layout, tabs, watermark, visual aid, import/export and local UI reset.

The settings center must derive its sections from enabled contracts:

- `GET /api/capabilities` exposes each capability's `configResources`;
- `resources/generated/admin-resource-contract.json` provides the schema, permissions, form layout and route metadata for those resources;
- capability manifests remain the source of truth for Admin resources, menus, permission prefixes, App routes, auth providers, service operations and demo datasets.

When a capability needs configuration, declare normal Admin resources under the owning manifest, keep them under the configuration/menu grouping when they are system-level settings, and let `/settings` link or embed those resources. Low-level CRUD resources may be hidden from the primary navigation if a workbench such as `/settings` or `/message-center` is the product entry, but they must remain present in the generated resource contract and reachable through the owning entry. Do not solve configuration discoverability by hard-coding a shell menu or by adding page-local API calls.

Settings runtime v1.1 exposes `GET /api/admin/settings`, `PUT /api/admin/settings/{resource}/{id}`, `POST /api/admin/settings/{resource}/{id}/validate-config` and `POST /api/admin/settings/{resource}/{id}/test-connect`. Runtime aggregation must include enabled `notification` configuration resources and `credential-auth-settings` when those capabilities are enabled. Protected provider credentials and other secret fields are write-only encrypted values: they may be submitted through the settings update endpoint only when the field schema marks them as encrypted secret fields, but they must never be returned by settings runtime, validation, test-connect, OpenAPI examples, audit payloads or Admin UI state. Validate/test-connect endpoints perform local schema and adapter preflight only; they must not require or contact real vendor accounts.

After enable/disable, the expected user-visible behavior is restart-bound: regenerate contracts, manually restart the API process, then verify that Admin resources, menu routes, permission codes, configuration sections, App routes, auth providers, demo datasets and service operations owned by that capability appear or disappear from the exposed contracts. While the running process differs from the desired capability set, the only valid UI state is `pendingRestart`.
...
Plugin-style capability governance has three machine gates: classification contracts, profile composition and operation policy. `resources/platform-capability-contracts.json` classifies every built-in, optional, local-demo and external-business capability, records its default/profile policy, owner boundary and replaceability, and keeps this governance metadata out of the runtime `capability.Manifest` struct. Verify the classification contract with:
...
The profile gate runs the same audit command for each declared composition, so missing dependencies, duplicate surfaces and business capability leakage are caught before runtime. `platform-default` must match `internal/platform/config.defaultCapabilities`; it must not include `external-business-capability`. `minimal-admin` proves the smallest admin shell can run without optional operations modules. `platform-app-ready` adds reusable app phone verification/binding. `platform-personnel-ready` adds the optional `personnel` capability without promoting HR-style resources into defaults. `scripts/validate-platform-personnel-runtime-readiness.mjs` is the runtime evidence gate for that profile: it rejects default admin contracts or audits that expose `personnel`, then verifies the personnel-ready contract produces personnel resources, read permissions, frontend entries and OpenAPI query paths/schemas together. `platform-notification-ready` adds the optional `notification` capability for the message-center workbench, reusable channel configuration, provider account contracts, send policies, templates, notification records and delivery ledgers without enabling concrete SMS/email/WeChat runtime adapters in the default base. `platform-job-ready` adds the optional `job` capability for job definitions, run records and attempt ledgers without bundling a worker engine, distributed lock or retry queue into the default base. The `external-business-capability` marker is a reference-classification placeholder only; it is not an executable profile and must not introduce business manifests into `platform-go`. When adding a capability, update this profile file only if the capability changes a reusable composition, then run the validator before updating docs or examples.

Capability lifecycle operations are governed by `resources/platform-capability-operation-policy.json` and verified by `scripts/validate-platform-capability-operation-policy.mjs`. The current platform is profile/config composed, not a runtime hot-plug marketplace: "install" means declaring a restart-required desired-state through a registered profile, `PLATFORM_CAPABILITIES`, `PLATFORM_CAPABILITY_LOCK_FILE` or a downstream composition root; contract regeneration plus a manual API process restart makes that desired set effective. "Disable" means removing the capability from the desired set, regenerating contracts and restarting; while the running process still differs from the desired set, the only supported state is `pendingRestart`, not hot apply. Destructive runtime uninstall, runtime source mutation, one-click remote repository pull and WebSocket progress are unsupported in v1. The policy marks `dictionary`, `tenant`, `identity`, `session`, `rbac`, `menu` and `admin-shell` as non-removable foundation capabilities. Other default or optional platform capabilities are disable-only with retained data that must become unreachable after restart: Admin resources, App routes, auth providers and demo datasets are explicitly checked as absent from exposed contracts. External business capability removal is downstream-owned and needs a reviewed migration/removal plan. Verify operation policy and supported compositions with:

```bash
rtk node scripts/validate-platform-capability-operation-policy.mjs
```

Plugin management v1 is a separate machine contract in `resources/platform-plugin-management-v1.json`. It productizes the same lifecycle as a restart-required desired-state model: choose the desired capability set through a profile, `PLATFORM_CAPABILITIES`, `PLATFORM_CAPABILITY_LOCK_FILE` or a downstream composition root, run the operation-policy/profile/contract gates, regenerate contracts, restart the API process manually and verify that exposed contracts match the desired set. v1 intentionally does not implement runtime hot install/uninstall, remote repository pull, destructive uninstall, source removal, data purge or WebSocket progress. Client-side update notices should use HTTP polling, `version.json` or an API version check, and should prompt for manual reload instead of refreshing dirty work automatically.

For a new business system, keep `platform-go` as the generic foundation and start the product-specific code in a downstream package, repository or composition root. The downstream project declares its capability manifest, resources, permissions, App routes, service contracts and lifecycle steps against `github.com/GnosiST/platform-go/pkg/platform/capability`, then binds every declared App route through `app.NewRouter` from `github.com/GnosiST/platform-go/pkg/platform/app`; storage models, business handlers, custom Admin panels, state machines and product tests stay downstream. The public App router accepts only a verified typed request identity from downstream middleware and does not export Gin, GORM, admin-resource or platform HTTP internals. `docs/external-business-project-template.md` and `examples/external-capability` are the maintained startup slice for that path. Promote code into `internal/platform` only when it is proven reusable across business domains and has its own platform capability contract. Verify the plugin management contract with:

```bash
rtk node scripts/validate-platform-plugin-management-v1.mjs
rtk node --test scripts/platform-plugin-management-v1.test.mjs
```
...
That gate checks three things: the example imports only public platform contracts, `go test ./...` passes inside the standalone module, and `go run .` emits a JSON-safe contract preview after registry validation. Manifests may contain Go functions for lifecycle hooks, so examples and downstream tooling must not JSON-marshal `capability.Manifest` directly; generate route/service/resource preview documents or other JSON-safe projections instead. The public package also exposes lifecycle history and recorded executor helpers (`NewMemoryLifecycleHistory`, `NewFileLifecycleHistory`, `NewSQLLifecycleHistory`, `NewGORMLifecycleHistory` and `NewRecordedLifecycleExecutor`) so downstream composition roots can run manifest migrations/seeds idempotently without importing `internal/platform/capability`. In a real business repository, keep the same boundary and add the missing runtime pieces locally: storage models, HTTP handlers, admin action handlers, service-object handlers, event/outbox binding, i18n dictionaries, migration/rollback evidence and acceptance tests.

Business onboarding should follow this sequence:

1. Declare the capability manifest and validate it with the public registry.
2. Declare admin resources, fields, deletion policy, ownership fields and data classification before writing UI.
3. Declare App routes and service contracts before binding handlers.
4. Add downstream persistence, migrations/seeds and rollback evidence in the business repository.
5. Run lifecycle migrations/seeds through a recorded executor backed by the deployment's chosen history store.
6. Generate or publish contract previews for Admin resources, App routes, services and OpenAPI.
7. Attach handlers through the downstream composition root without importing the business package from platform core.
8. Run the example gate, capability validators, route handler coverage and product-specific tests before enabling the capability in a profile.

The platform standard resources have extra governance gates in `scripts/validate-admin-resources.mjs`: tenant rows expose optional `areaCode`; user rows require `tenantCode` and expose optional `orgUnitCode`, optional `areaCode` and multirole `roles`; org units carry required `type`, required `tenantCode`, tree `parentCode` and optional `areaCode`; area codes carry tree `parentCode`, data-driven `level`, numeric `depth`, `path` and source provenance fields `sourceSystem`, `sourceCode`, `dataSet` and `metadata`; roles carry `groupCode`, required `dataScope`, custom org and area scope relations, and permission relations. `resources/platform-governance-topology.json` adds the architecture-level topology gate for the same model: `org-units` must stay a default multi-level tree, tenant `areaCode` must stay optional by default, organization units must be tenant-owned, role groups must not own membership or policy, `area-codes.level` must stay data-driven instead of fixed platform enum values, default profiles must exclude `personnel`, `platform-personnel-ready` must include the shared tenant, org, area and user resources, and its generated personnel admin contract must keep the declared personnel/position relation fields. The topology gate also treats `tenants`, `org-units`, `users`, `roles`, `role-groups` and `area-codes` as non-droppable default governance primitives, separately asserting that `area-codes` is a default resource while `areaCode` attachment fields are not mandatory, and rejects default-foundation promotion of optional personnel resources. These checks keep the base from shrinking back to tenant-only ownership while still letting business resources opt into only the ownership dimensions they actually need.
