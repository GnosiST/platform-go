...
The current platform intentionally has no legacy local-password provider kind wired into HTTP login. The API composition root rejects the explicit provider kind `password`; provider IDs and generic field names do not select authentication semantics. `credential-auth` owns the separate Argon2id service foundation and uses explicit `credential-password` / `credential-sms-otp` provider kinds for its persistent-runtime-p0. It must still add reviewed challenge, migration/reset/rotation and production governance contracts before it can be promoted as production-complete; it must not persist password hashes in generic `Record.Values` or make hashing policy a mutable project-initialization switch.

### credential-auth capability rules

`credential-auth` is the local credential authentication capability for username/password, phone/password, email/password and phone/SMS OTP login. The current package is `persistent-runtime-p0`: `resources/platform-credential-auth-v1.json`, `scripts/validate-platform-credential-auth-v1.mjs`, `scripts/platform-credential-auth-v1.test.mjs`, `internal/platform/credentialauth`, `internal/platform/httpapi` and `admin/src/platform/auth` define the boundary, provider declarations, internal service foundation, dedicated GORM credential repository, application-layer hybrid encrypted secret transport, HTTP login slice and provider-driven Admin login UI. It is not production-complete.

Do not declare provider kind `password` in a manifest. The current `cmd/platform-api` guard must keep rejecting `password` provider kind; credential-auth provider declarations use the explicit `credential-password` and `credential-sms-otp` kinds. Existing demo, Admin OIDC and app provider login flows must remain compatible while the credential-auth API contract is `persistent-runtime-p0`.

Do not store password hashes, raw passwords, OTP codes, challenge answers or credential reset material in generic `Record.Values`. `credential-auth` must use dedicated stores for `auth_identifiers`, `password_credentials`, `credential_challenges` and `sms_otp_challenges`; generic resources may only expose masked or verified identifier metadata when the owning capability explicitly declares sensitivity, storage and projection rules. Password verification uses Argon2id with a versioned parameter policy and rehash-on-upgrade behavior. Browser clients must not submit local passwords or OTP codes as plaintext JSON; they fetch `GET /api/auth/credential-secret-key`, use ECDH P-256 to agree on a shared key, derive with HKDF-SHA256, encrypt the password or SMS OTP secret with AES-256-GCM/A256GCM, and submit an ECDH-P256-HKDF-SHA256+A256GCM `secret.encrypted` envelope to `POST /api/auth/login`. When encrypted secrets are required, `secret.value` and `secret.code` are invalid even if `secret.encrypted` is also present. HTTPS remains required in production, but it is not the application-layer credential secret encryption mechanism and cannot be used as a substitute for `secret.encrypted`.

SMS OTP is an authentication secret type owned by `credential-auth`, but SMS delivery is an extension of the reusable `notification` capability. The current notification SMS foundation provides the sender port, `mock-local` development/test sender, provider canonicalization and config validation that rejects mock SMS outside development/test. Aliyun/Tencent can be assembled as non-network dry-run senders with placeholder credentials for configuration validation and receipt generation; live vendor sending, delivery ledgers, vendor error redaction and channel-level limits belong to `notification`, not to a business package or the Admin shell. Production credential-auth requires the dedicated repository and secret transport key configuration; it remains not production-complete until challenge, live SMS and governance evidence are complete.
