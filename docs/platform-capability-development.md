...
`credential-auth` is the planned local credential authentication capability for username/password, phone/password, email/password and phone/SMS OTP login. The first approved package is contract-only: `resources/platform-credential-auth-v1.json`, `scripts/validate-platform-credential-auth-v1.mjs` and `scripts/platform-credential-auth-v1.test.mjs` define the boundary before backend runtime is added.

Do not declare provider kind `password` in a manifest until the credential-auth runtime package deliberately changes the API composition root and preserves the compatibility contract. The current `cmd/platform-api` guard must keep rejecting `password` provider kind while this contract is in the `specified-not-implemented` state. Existing demo, Admin OIDC and app provider login flows must remain unchanged.
...
SMS OTP is an authentication secret type owned by `credential-auth`, but SMS delivery is an extension of the reusable `notification` capability. Provider adapters, templates, delivery ledgers, vendor error redaction, channel-level limits and production mock-provider rejection belong to `notification`, not to a business package or the Admin shell.
