...
The current platform intentionally has no local-password provider wired into HTTP login. The API composition root rejects the explicit provider kind `password`; provider IDs and generic field names do not select authentication semantics. `credential-auth` owns the separate Argon2id service foundation and must later add reviewed migration/reset/rotation contracts before it can be wired into runtime login; it must not persist password hashes in generic `Record.Values` or make hashing policy a mutable project-initialization switch.

### credential-auth capability rules

`credential-auth` is the planned local credential authentication capability for username/password, phone/password, email/password and phone/SMS OTP login. The current package is service-foundation-not-wired: `resources/platform-credential-auth-v1.json`, `scripts/validate-platform-credential-auth-v1.mjs`, `scripts/platform-credential-auth-v1.test.mjs` and `internal/platform/credentialauth` define the boundary and first internal service foundation before HTTP login is added.

Do not declare provider kind `password` in a manifest until the credential-auth runtime package deliberately changes the API composition root and preserves the compatibility contract. The current `cmd/platform-api` guard must keep rejecting `password` provider kind while the HTTP API contract is `specified-not-implemented`. Existing demo, Admin OIDC and app provider login flows must remain unchanged.
...
Future Admin login UI must use `GET /api/auth/providers` discovery and render provider modes from the returned declarations. Do not hard-code the four credential modes into shell layout logic; provider-specific state machines belong behind the capability contract and shared auth UI components.
