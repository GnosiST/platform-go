...
Capability lifecycle operations are governed by `resources/platform-capability-operation-policy.json` and verified by `scripts/validate-platform-capability-operation-policy.mjs`. The current platform is profile/config composed, not a runtime hot-plug marketplace: "install" means enabling a registered manifest through a profile, `PLATFORM_CAPABILITIES`, `PLATFORM_CAPABILITY_LOCK_FILE` or a downstream composition root before startup and contract generation; "disable" means removing it from the enabled set, regenerating contracts and restarting; destructive runtime uninstall is unsupported. The policy marks `dictionary`, `tenant`, `identity`, `session`, `rbac`, `menu` and `admin-shell` as non-removable foundation capabilities. Other default or optional platform capabilities are disable-only with retained data that must become unreachable through schemas, menus, providers, routes and demo datasets. External business capability removal is downstream-owned and needs a reviewed migration/removal plan. Verify operation policy and supported compositions with:

```bash
rtk node scripts/validate-platform-capability-operation-policy.mjs
```

Plugin management v1 is a separate machine contract in `resources/platform-plugin-management-v1.json`. It productizes the same lifecycle as a restart-required desired-state model: choose the desired capability set through a profile, `PLATFORM_CAPABILITIES`, `PLATFORM_CAPABILITY_LOCK_FILE` or a downstream composition root, regenerate contracts, restart the API process manually and verify that exposed contracts match the desired set. v1 intentionally does not implement runtime hot install/uninstall, remote repository pull, destructive uninstall, source removal, data purge or WebSocket progress. Client-side update notices should use HTTP polling, `version.json` or an API version check, and should prompt for manual reload instead of refreshing dirty work automatically.
