...
The current platform intentionally has no legacy local-password provider kind wired into HTTP login. The API composition root rejects the explicit provider kind `password`; provider IDs and generic field names do not select authentication semantics. `credential-auth` owns the separate Argon2id service foundation and uses explicit `credential-password` / `credential-sms-otp` provider kinds for its partial development runtime. It must still add reviewed persistent storage, challenge, migration/reset/rotation and production governance contracts before it can be promoted as production-complete; it must not persist password hashes in generic `Record.Values` or make hashing policy a mutable project-initialization switch.

### credential-auth capability rules

`credential-auth` is the local credential authentication capability for username/password, phone/password, email/password and phone/SMS OTP login. The current package is `dev-http-runtime-memory-bootstrap`: `resources/platform-credential-auth-v1.json`, `scripts/validate-platform-credential-auth-v1.mjs`, `scripts/platform-credential-auth-v1.test.mjs`, `internal/platform/credentialauth`, `internal/platform/httpapi` and `admin/src/platform/auth` define the boundary, provider declarations, first internal service foundation, partial HTTP login slice and provider-driven Admin login UI. It is not production-complete.

Do not declare provider kind `password` in a manifest. The current `cmd/platform-api` guard must keep rejecting `password` provider kind; credential-auth provider declarations use the explicit `credential-password` and `credential-sms-otp` kinds. Existing demo, Admin OIDC and app provider login flows must remain compatible while the credential-auth API contract is `implemented-partial`.

Do not store password hashes, raw passwords, OTP codes, challenge answers or credential reset material in generic `Record.Values`. `credential-auth` must use dedicated stores for `auth_identifiers`, `password_credentials`, `credential_challenges` and `sms_otp_challenges`; generic resources may only expose masked or verified identifier metadata when the owning capability explicitly declares sensitivity, storage and projection rules. Password verification uses Argon2id with a versioned parameter policy and rehash-on-upgrade behavior.

SMS OTP is an authentication secret type owned by `credential-auth`, but SMS delivery is an extension of the reusable `notification` capability. The current notification SMS foundation provides the sender port, `mock-local` development/test sender, provider canonicalization and config validation that rejects mock SMS outside development/test. Concrete Aliyun/Tencent adapters, delivery ledgers, vendor error redaction and channel-level limits belong to `notification`, not to a business package or the Admin shell. Production credential-auth remains blocked until persistent credential storage and production SMS/governance evidence exist.
