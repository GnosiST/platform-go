...
Admin login UI must use `GET /api/auth/providers` discovery and render provider modes from the returned declarations. Do not hard-code the four credential modes into shell layout logic; provider-specific state machines belong behind the capability contract and shared auth UI components. The current UI slice covers credential password, phone SMS OTP and CAPTCHA login challenge flows; a real slider component remains future UI work.
