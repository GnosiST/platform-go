...
3. Any future visual candidate requires an explicit task-graph node, design gates, resource locks and fresh screenshot evidence. Candidate A and production Admin OIDC remain closed in the original 37-node baseline; `admin-watermark-export-governance` and `sensitive-data-reveal-step-up` are implemented, while the organization Admin UX lane and `public-docs-site` remain controlled pending work in the active `66 total / 46 implemented / 20 controlled unfinished` completion program.
