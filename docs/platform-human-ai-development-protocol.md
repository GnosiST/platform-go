...
Capability install, disable and uninstall decisions must use `resources/platform-capability-operation-policy.json`. The platform supports restart-required desired-state composition, not runtime hot-plugging: enabling a capability means selecting a registered manifest through a profile, `PLATFORM_CAPABILITIES`, `PLATFORM_CAPABILITY_LOCK_FILE` or a downstream composition root, regenerating contracts and manually restarting the API process. Disabling a default or optional capability means removing it from the desired set, regenerating contracts and restarting; until restart, the mismatch is only `pendingRestart`. After restart, disabled Admin resources, App routes, auth providers and demo data sets must not remain exposed. Foundation capabilities are non-removable. Runtime source mutation, one-click remote repository pull, destructive data purge or source-package removal is not a generic platform operation and requires reviewed migration, rollback and owner evidence.

`resources/platform-plugin-management-v1.json` defines the productized plugin management v1 contract. It is a restart-required desired-state model: humans or AI agents declare the desired capability set through a profile, `PLATFORM_CAPABILITIES`, `PLATFORM_CAPABILITY_LOCK_FILE` or a downstream composition root, run the contract/profile/operation-policy gates, regenerate artifacts, then manually restart the API process. v1 does not support runtime hot install/uninstall, remote repository pull, destructive uninstall, source removal, data purge or WebSocket progress. Update detection should be HTTP polling, `version.json` or API version checks with a manual reload prompt; frontends may prompt reload, but must not silently refresh dirty work.

System-level function configuration has one product entry: `/settings`. It dynamically groups enabled capability configuration resources from manifest projection, especially `configResources` returned by `GET /api/capabilities` and schemas from `resources/generated/admin-resource-contract.json`. Do not add a new top-level menu every time a capability needs configuration. The topbar settings button is only for interface preferences such as theme, density, layout, tabs, watermark and import/export of local UI preferences.

After a capability is enabled or disabled and the API process is manually restarted, the exposed surfaces must match the resolved manifests. Admin resources, menu routes, permission codes, configuration resources, App routes, auth providers, demo datasets and service operations appear or disappear through the capability manifest and generated contracts. A capability is not product-ready just because it appears in the install list; it must also have a reachable menu, settings entry, provider discovery entry or service contract appropriate to what the manifest declares.
