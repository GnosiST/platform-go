...
- The organization target contract is now frozen in `docs/platform-organization-rbac-menu-contract.md`, but it is deliberately not described as implemented runtime behavior. It requires one primary organization for each ordinary user, derives tenant ownership server-side from that organization, removes editable tenant selection from the user workflow, and defines an explicit exception for platform-level users without an organization. An organization may bind multiple enabled role groups from the same tenant. Role groups form a strict non-nested `role group -> role` tree, each role belongs to exactly one role group, and the organization's effective role pool is the deduplicated union of enabled roles in its enabled directly bound groups. Every user role assignment, including create, update, import and batch operations, must be a subset of that pool and must be enforced by the backend.
...
- Until the organization migration nodes replace this surface, user role bindings use the current `users.roles` multiselect relation and the backend accepts legacy `role` values for older snapshots. The activated target keeps multi-role selection but makes its options organization-dependent and backend-validated.
