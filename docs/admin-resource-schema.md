...
- The migration-source RBAC schema still exposes `role-groups.parentCode`, assigns roles through `roles.groupCode`, binds users through `users.roles`, and derives menu visibility from role permissions. Nested groups and direct generic role-policy fields are compatibility inputs only. Roles own allow permissions, deny permissions and data scope; role groups are classification-only and do not grant or inherit authorization.
- The organization target backend in `docs/platform-organization-rbac-menu-contract.md` is implemented behind `PLATFORM_ORGANIZATION_RBAC_MODE=target` and required in production. It requires one primary organization for each ordinary user, derives tenant ownership server-side from that organization, and defines an explicit exception for platform-level users without an organization. An organization may bind multiple enabled role groups from the same tenant. Role groups form a strict non-nested `role group -> role` tree, each role belongs to exactly one role group, and the organization's effective role pool is the deduplicated union of enabled roles in its enabled directly bound groups. Backend create/update, relation mutation, service-command and migration paths enforce that user roles remain a subset of the pool. `organization-user-admin-experience` implements organization-dependent role selectors, and `role-tree-and-authorization-entry` implements the dedicated strict role workbench plus atomic allow/deny/data-scope authorization. Target mode rejects generic writes that bypass those domain-owned paths.
- The activated menu target separates directory menus from routable page menus and persists role-to-menu visibility independently from API and page-button permissions. The menu schema, page-button metadata, page-only `role_menu` persistence seams and revision-aware target resolution are implemented behind closed serving and write gates. Roles continue to own allow/deny permissions and data scope, while Casbin remains the final protected-API boundary. Organization changes, role-group unbinding, role movement or disablement, and migration from permission-derived menus require explicit conflict handling and audit; they must not silently retain invalid grants or discard existing access.
...
- current validators intentionally preserve migration-source fields: `users.tenantCode` and `org-units.tenantCode` are required, `users.orgUnitCode` remains optional, nested `role-groups.parentCode` is accepted only as legacy compatibility, and `roles.groupCode`, role permission and data-scope fields remain hard gates. Target-mode runtime validators reject nested groups and bypass writes;
...
- organization/user, role, menu and E2E completion gates are implemented: ordinary users require one primary organization, tenant is derived with same-tenant integrity, role groups are non-nested, each role belongs to one group, organizations bind multiple same-tenant groups, user roles stay inside the effective pool, and role policy changes use atomic domain commands. Independent page-only role-menu bindings, directory/page schema, page-button metadata, revision-aware navigation service objects and the dedicated Admin editor are implemented. Scale, principal dual-read and cutover/rollback behavior are certified by the organization E2E evidence; production serving and role-menu writes remain default-closed pending explicit rollout approval, without weakening API permission enforcement;
