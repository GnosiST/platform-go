...
- organization/user and role completion gates are implemented: ordinary users require one primary organization, tenant is derived with same-tenant integrity, role groups are non-nested, each role belongs to one group, organizations bind multiple same-tenant groups, user roles stay inside the effective pool, and role policy changes use atomic domain commands. The remaining menu gates must add independent role-menu bindings, directory/page schema, page-button configuration and migration/cutover evidence without weakening API permission enforcement;
