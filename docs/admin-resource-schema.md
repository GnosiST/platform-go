...
- organization/user, role and menu completion gates are implemented: ordinary users require one primary organization, tenant is derived with same-tenant integrity, role groups are non-nested, each role belongs to one group, organizations bind multiple same-tenant groups, user roles stay inside the effective pool, and role policy changes use atomic domain commands. Independent page-only role-menu bindings, directory/page schema, page-button metadata, revision-aware navigation service objects and the dedicated Admin editor are implemented behind closed cutover gates. Remaining work is full-scale role assignment, principal dual-read, migration/cutover certification and organization E2E without weakening API permission enforcement;
