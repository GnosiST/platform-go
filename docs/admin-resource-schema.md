...
- The legacy Admin surface still uses the current `users.roles` multiselect relation and accepts legacy `role` values for compatibility snapshots. Target mode keeps multi-role selection backend-validated; organization-dependent options and explicit invalid-role remediation remain pending in `organization-user-admin-experience`.
