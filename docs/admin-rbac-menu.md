...
The `permissions` resource is a governed catalog from enabled capability manifests, registered platform control-plane permissions and hand-maintained custom API permissions. The legacy `roles` schema keeps `permissions` and `denyPermissions` fields for compatibility, while target mode uses the dedicated Tree Transfer and reviewed domain command instead of generic role mutation. API and page-button permissions are grouped separately; disabled or missing historical entries can be removed from role assignments but not newly assigned.

The Admin `/permissions` route renders `PermissionGovernanceConsole` instead of the generic resource console, grouping permission records as resource type -> capability -> resource -> permission code. System-generated records from capability manifests, resource schemas and page-button configuration are locked in this view. Operators with `admin:permission:create` or `admin:permission:update` can register and maintain custom API permissions marked as `capability=custom-api`; those entries then become available to role authorization through the same permission catalog. Hard deletion is not exposed from this workbench because permission records can be referenced by existing roles; use disablement for retirement.
