...
const settingsDrawerCall = sourceRange(files.shell, "<SystemSettingsDrawer", "      />");
...
requireNotIncludes(files.settings, "AdminCurrentSession", "Topbar interface settings must not receive user session data.");
requireNotIncludes(files.settings, "BrandingConfig", "Topbar interface settings must not receive platform branding or system metadata.");
requireNotIncludes(files.settings, "settings-profile-card", "Topbar interface settings must not render user profile information.");
requireNotIncludes(files.settings, "dictionary.runtimeContext", "Topbar interface settings must not render runtime or tenant context.");
requireNotIncludes(files.settings, "dictionary.environmentContext", "Topbar interface settings must not render environment context.");
requireNotIncludes(files.settings, "dictionary.tenantContext", "Topbar interface settings must not render tenant context.");
requireNotIncludes(files.settings, "onLogout", "Logout must live in the personal profile menu, not the interface settings drawer.");
...
requireIncludes(files.shell, "profile-summary-actions", "Profile summary dropdown must expose grouped account actions.");
requireIncludes(files.shell, "dictionary.logout", "Profile summary dropdown must expose logout in the account menu.");
requireIncludes(files.shell, "LogoutOutlined", "Profile summary logout action must use the logout icon.");
...
requireNotIncludes(settingsDrawerCall, "onLogout={onLogout}", "Interface settings drawer must not receive the logout action from AdminShell.");
...
requireCssRule(files.styles, ".profile-summary-actions", ["display: grid;", "gap: 8px;"], "Profile summary account actions must keep a stable stacked layout.");
requireIncludes(files.dashboard, 'className: "dashboard-table-title-cell"', "Dashboard plugin title column must preserve emphasis without rendering React nodes in rc-table cells.");
requireNotIncludes(files.dashboard, "render: (value: LocalizedText) => <Typography.Text strong>", "Dashboard table cells must not render React elements that trigger rc-table circular-reference warnings.");
